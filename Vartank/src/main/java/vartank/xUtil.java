...
public class xUtil {
    ...
    // BFS - алгоритм пошуку шляху в ширину
    ...
    // DFS - алгоритм пошуку шляху в глибину
    ...
    // хвильовий алгоритм для пошуку найкоротшого шляху
    public static boolean ShortestWave(XField field, int start_x, int start_y, int end_x, int end_y, Vector<Integer> path){
        ...
        // спочатку розмітимо всі доступні комірки по наростаючій хвилі
        for (boolean loop = true; loop; ++cnt) {
            ...
            for (int r = 0; r < field.GetHeight(); r++) {
                ...
                for (int c = 0; c < field.GetWidth(); c++) {
                    if (vis[row + c] != cnt)
                        continue;
                    for (int i = 0; i < dirs.length; i += 2) { // перегляд в різні сторони
                        nx = c + dirs[i];
                        ny = r + dirs[i + 1];
                        if ((nx < 0) || (ny < 0) || (nx >= field.GetWidth()) || (ny >= field.GetHeight()))
                            continue;

                        if (vis[ny * field.GetWidth() + nx] == 0) { // якщо шлях вільний
